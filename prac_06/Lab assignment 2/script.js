...
function draw() {
    ...
    for (let i = 0; i < poses.length; i++) {
        ...
        // Indices for left leg: Hip = 11, Knee = 13, Ankle = 15
        let leftHip = pose.keypoints[11];
        let leftKnee = pose.keypoints[13];
        let leftAnkle = pose.keypoints[15];
        
        // Check if we have high enough confidence for all 3 points
        if (leftHip.confidence > 0.1 && leftKnee.confidence > 0.1 && leftAnkle.confidence > 0.1) {
            let angle = calculateAngle(leftHip, leftKnee, leftAnkle);
            
            // Display angle
            document.getElementById('knee-angle').innerText = Math.round(angle) + '°';
            
            // Squat state machine
            if (angle < 100 && !isSquatting) {
                isSquatting = true;
            } else if (angle > 160 && isSquatting) {
                isSquatting = false;
                squatCount++;
                document.getElementById('squat-count').innerText = squatCount;
            }
            
            // Highlight the knee being used for calculation
            fill(245, 158, 11); // amber
            noStroke();
            ellipse(leftKnee.x, leftKnee.y, 20, 20);
        } else {
            document.getElementById('knee-angle').innerText = '-°';
        }
        // ----------------------------
